...
//checks if the user doesnt have low quality option on
	...
	var mist4 = new FlxSprite(-700, -270).loadGraphic(Paths.image(ext + 'mistBack'));
	...
	var mist5 = new FlxSprite(-200, -270).loadGraphic(Paths.image(ext + 'mistMid'));
	...
	if (hasBfSkin && game.boyfriend.curCharacter != 'detectiveplayer' && game.boyfriend.curCharacter != 'bobby')
	{
		triggerEventNote('Change Character', 'boyfriend', 'picoweird');
	}
}
